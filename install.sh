...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
##@Version           :  202602020700-git
# @@Author           :  Jason Hempstead
# @@Contact          :  jason@casjaysdev.pro
# @@License          :  WTFPL
# @@ReadME           :  install.sh --help
# @@Copyright        :  Copyright: (c) 2026 Jason Hempstead, Casjays Developments
# @@Created          :  Sunday, Feb 02, 2026 07:00 EDT
# @@File             :  install.sh
# @@Description      :  Install configurations for zellij
...
__run_prepost_install() {
  local getRunStatus=0
  # Install zellij binary from GitHub releases if not available via package manager
...
__install_zellij_binary() {
  local ZELLIJ_BIN="/usr/local/bin/zellij"
  local ZELLIJ_REPO="zellij-org/zellij"
  local ZELLIJ_TMP="/tmp/zellij-install-$$"
  ...
  ZELLIJ_VERSION=$(curl -q -LSsf "https://api.github.com/repos/${ZELLIJ_REPO}/releases/latest" 2>/dev/null | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
  [ -z "$ZELLIJ_VERSION" ] && return 1
  # Detect architecture
  case "$(uname -m)" in
  ...
  *) return 1 ;;
  ...
  case "$(uname -s)" in
  Linux) ZELLIJ_OS="unknown-linux-musl" && ZELLIJ_FILE="zellij-${ZELLIJ_ARCH}-${ZELLIJ_OS}.tar.gz" ;;
  Darwin) ZELLIJ_OS="apple-darwin" && ZELLIJ_FILE="zellij-${ZELLIJ_ARCH}-${ZELLIJ_OS}.tar.gz" ;;
  MINGW* | MSYS* | CYGWIN*) ZELLIJ_OS="pc-windows-msvc" && ZELLIJ_FILE="zellij-${ZELLIJ_ARCH}-${ZELLIJ_OS}.zip" ;;
  *) return 1 ;;
  esac
  ZELLIJ_URL="https://github.com/${ZELLIJ_REPO}/releases/download/${ZELLIJ_VERSION}/${ZELLIJ_FILE}"
  # Create temp directory and download
  mkdir -p "$ZELLIJ_TMP" || return 1
  curl -q -LSsf "$ZELLIJ_URL" -o "$ZELLIJ_TMP/$ZELLIJ_FILE" 2>/dev/null || { rm -rf "$ZELLIJ_TMP"; return 1; }
  # Extract the binary
  cd "$ZELLIJ_TMP" || return 1
  case "$ZELLIJ_FILE" in
  *.tar.gz) tar -xzf "$ZELLIJ_FILE" 2>/dev/null || { rm -rf "$ZELLIJ_TMP"; return 1; } ;;
  *.zip) unzip -q "$ZELLIJ_FILE" 2>/dev/null || { rm -rf "$ZELLIJ_TMP"; return 1; } ;;
  esac
  # Install the binary
  if [ -f "$ZELLIJ_TMP/zellij" ]; then
    chmod +x "$ZELLIJ_TMP/zellij"
    if [ -w "/usr/local/bin" ]; then
      mv -f "$ZELLIJ_TMP/zellij" "$ZELLIJ_BIN"
    else
      sudorun mv -f "$ZELLIJ_TMP/zellij" "$ZELLIJ_BIN"
    fi
  fi
  # Cleanup
  rm -rf "$ZELLIJ_TMP"
  cd - >/dev/null 2>&1 || true
  __cmd_exists zellij && return 0 || return 1
}
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
